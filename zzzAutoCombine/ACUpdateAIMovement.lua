...
function AutoCombine:acUpdateAIMovement(superFunc, dt)
	...
	if self.driveBackPosX == nil then
		...
		if self.turnStage == 0 or self.turnStage >= 20 then
...
--==============================================================		
		...
		--AutoCombineHud.setInfoText(self, string.format("turn angle = %3i",turnAngle))
...
-- The U-turn					
		elseif self.acTurnStage == 17 then
			...
			--	ref = -120
			--elseif not self.acParameters.noReverse then
				ref = self.acDimensions.uTurnRefAngle
			--end
			...
			if self.acBorderDetected then
				...
				self.turnTimer      = self.acDeltaTimeoutRun;
			elseif not self.acParameters.noReverse and turnAngle <= ref then
				self.acTurn2Outside   = true;
				self.acTurnStage      = 18;
				self.turnTimer        = self.acDeltaTimeoutStop;
